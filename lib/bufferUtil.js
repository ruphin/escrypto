...
const spaceShip = (bufferA, bufferB) => {
  ...
  // Check if some byte in longArray is not equal to the same byte in shortArray
  longArray.some((value, index) => {
    if (index < lengthDifference) {
      // If index < lengthDifference we are checking leading bytes
      if (value === 0) {
        // While leading bytes are zero, the buffers are still equal
        return false;
      }
      // If longArray has a non-zero leading byte, it is bigger
      result = LongArrayIsBufferA ? 1 : -1;
      return true;
    }
    // This is not a leading byte
    // If the same byte in shortArray has the same value, the buffers are still equal
    if (value === shortArray[index - lengthDifference]) {
      return false;
    }
    // If the bytes have different values, one is bigger
    result = value > shortArray[index - lengthDifference] === LongArrayIsBufferA ? 1 : -1;
    return true;
  });
...
export const sub = (bufferA, bufferB) => {
  if (gt(bufferB, bufferA)) {
    throw new OverflowError('Result is smaller than 0');
  }

  const output = new ArrayBuffer(bufferA.byteLength);
};
...
console.log(gt(a, b));
console.log(lte(b, b));
console.log(eq(fromHex('0x00ab'), fromHex('0xab')));
console.log(lt(fromHex('0xab'), fromHex('0xac')));
