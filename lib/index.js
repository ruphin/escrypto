...
const BITCOIN_SEED = bufferUtil.fromUTF('Bitcoin seed');
...
export const mnemonicToSeed = mnemonic => {
  const passphrase = bufferUtil.fromUTF(mnemonic.normalize('NFKD'));
  const salt = bufferUtil.fromUTF('mnemonic');
...
class EllipticCurve {
  ...
  testPoint(point) {
    return point.y.mul(point.y).eq(
      point.x
        .mul(point.x)
        .mul(point.x)
        .add(this.a.mul(point.x))
        .add(this.b)
    );
  }
}
...
class Point {
  constructor(x, y, curve) {
    ...
    if (!curve.testPoint(this)) {
      throw 'NOT A POINT ON THIS CURVE';
    }
  }
}

let curve = new EllipticCurve(new Uint(1), new Uint(2));
let point = new Point(new Uint(1), new Uint(2), curve);
...
class MyButton extends GluonElement {
  ...
  attributeChangedCallback(attr, oldValue, newValue) {
    if (attr === 'loading' && oldValue !== newValue) {
      this.render();
    }
  }
}
